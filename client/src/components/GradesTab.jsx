...
import jsPDF from "jspdf"; // 👈 NEW: The PDF Generator
import autoTable from "jspdf-autotable"; // 👈 NEW: The Table Maker for the PDF

const GradesTab = ({ isAdmin, students, subjects }) => {
  ...
  // --- NEW: Bulletproof PDF Generation Function ---
  const downloadPDF = () => {
    try {
      const doc = new jsPDF();

      // Safely figure out the name to put on the PDF
      let studentName = "My";
      if (isAdmin && selectedReportStudent) {
      ...
      // 4. Draw the Table (Using the modern autoTable syntax!)
      autoTable(doc, {
        head: [tableColumn],
        body: tableRows,
        startY: 48,
      ...
      alert(
        "❌ Something went wrong while generating the PDF. Check the browser console!",
      );
    }
  };
      ...
      {isAdmin && (
        <div className="bg-purple-50 dark:bg-gray-700 p-6 rounded-xl border border-purple-100 dark:border-gray-600 shadow-sm">
          ...
          >
            ...
            <button
              type="submit"
              className="md:col-span-3 mt-2 px-6 py-3 bg-purple-600 text-white font-bold rounded-lg w-full md:w-auto shadow-md"
            >
              + Save Grade
            </button>
          </form>
      ...
      <div>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-2 mb-4 dark:border-gray-700">
          <h4 className="text-xl font-bold">
            {isAdmin ? "View Student Report Card" : "My Report Card"}
          </h4>
          ...
          {reportCard.length > 0 && (
            <button
              onClick={downloadPDF}
              className="mt-2 md:mt-0 px-4 py-2 bg-gray-800 dark:bg-gray-200 text-white dark:text-gray-900 font-bold rounded shadow hover:bg-gray-700 dark:hover:bg-white transition-colors"
            >
              📄 Download PDF
            </button>
          )}
        ...
        {isAdmin && !selectedReportStudent ? null : (
