...
const ChatTab = ({ userData }) => {
  ...
  useEffect(() => {
    ...
    const newSocket = io(SOCKET_URL, { auth: { token }, reconnection: true });
    setSocket(newSocket);
  ...
  useEffect(() => {
    ...
    const loadHistoryAndJoin = async () => {
      ...
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(`${API_URL}/chat/history/${room}`, {
  ...
  return (
    <div className="animate-fade-in flex flex-col h-[600px] bg-white dark:bg-gray-800 rounded-xl shadow-md border dark:border-gray-700 overflow-hidden">
      ...
      <div className="bg-blue-600 p-4 text-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 z-10">
        <h4 className="text-xl font-bold flex items-center gap-2">Live Communications</h4>
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-gray-50 dark:bg-gray-900 space-y-4">
        {historyLoading ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="italic font-medium text-sm">Welcome to {room}. Say hello!</p>
          </div>
        ) : (
          messageList.map((msg, index) => {
            ...
            return (
              <div key={msg.id || index} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[75%] rounded-2xl p-4 shadow-sm ${isMe ? "bg-blue-600 text-white rounded-br-none" : "bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-bl-none"}`}>
                  <div className="flex justify-between items-baseline mb-1 gap-4">
                    <span className={`text-xs font-bold ${isMe ? "text-blue-200" : "text-gray-500"}`}>{isMe ? "You" : msg.sender} ({msg.role})</span>
                    <span className={`text-[10px] ${isMe ? "text-blue-200" : "text-gray-400"}`}>{msg.time}</span>
                  </div>
      ...
      <form onSubmit={sendMessage} className="p-4 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 flex gap-3">
        <input
          type="text"
        ...
        />
        <button type="submit" disabled={!currentMessage.trim()} className="bg-blue-600 text-white p-3 rounded-full hover:bg-blue-700 disabled:bg-gray-300 dark:disabled:bg-gray-700 shadow-md flex items-center justify-center w-12 h-12 shrink-0 transition-colors">
