...
// Helper function to turn SQL timestamps into "10 mins ago"
...
// Maps backend audit_log actions to beautiful UI elements
const getActionDetails = (log) => {
  const name = log.full_name;
  const val =
  ...
  switch (log.action) {
    ...
    case "UPLOAD_DOCUMENT":
      return {
        text: `${name} uploaded ${val.title || "a document"}`,
        icon: FileText,
        color: "text-blue-500",
    ...
    case "CREATE_STUDENT":
      return {
        text: `${name} enrolled ${val.full_name || "a student"}`,
        icon: Users,
    ...
    case "UPDATE_GRADE":
      return {
        text: `${name} updated academic results`,
        icon: Award,
    ...
    default:
      return {
        text: `${name} performed a system action`,
        icon: Settings,
        color: "text-gray-500",
    ...
    const fetchActivity = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (isMounted) {
            const mappedActivities = data.map((log) => ({
              id: log.log_id,
          ...
          {isAdmin && (
            <div>
              ...
              <button className="w-full mt-5 py-2 text-xs font-bold text-gray-500 hover:text-gray-800 dark:hover:text-white transition-colors flex items-center justify-center gap-1">
                Refresh Activity <ChevronRight size={14} />
              </button>
            </div>
          )}
            ...
            <div>
              ...
              <div className="space-y-3">
                {studentTasks.map((task) => (
                  <div
                    key={task.id}
                    className="p-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-sm flex items-start gap-3"
                  >
                    <div className="mt-0.5">
                      <div
                        className="w-4 h-4 rounded border-2 border-gray-300 dark:border-gray-600 cursor-pointer hover:border-blue-500 transition-colors"
                        aria-label={`Mark task ${task.text} complete`}
                        role="checkbox"
                        aria-checked="false"
                        tabIndex="0"
                      ></div>
                    ...
                    <div>
                      ...
                      <p
                        className={`text-xs mt-1 font-medium ${task.urgent ? "text-red-500" : "text-gray-500"}`}
                      >
                        Due: {task.due}
                      </p>
                    </div>
