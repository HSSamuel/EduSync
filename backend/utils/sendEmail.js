...
async function buildTransportConfig() {
  ...
  if (!EMAIL_FORCE_IPV4) {
    ...
    if (EMAIL_ALLOW_INVALID_TLS) config.tls = { rejectUnauthorized: false };
    return config;
  }
...
async function getTransporter() {
  if (!transporterPromise) {
    transporterPromise = buildTransportConfig().then((transportConfig) =>
...
const sendEmail = async ({ to, subject, html, text }) => {
  try {
    ...
    // Fix: Invalidate cached SMTP IPs on network errors so it resolves fresh next time
    resetEmailTransport();
    throw error;
  }
};
...
module.exports.resetEmailTransport = resetEmailTransport;
