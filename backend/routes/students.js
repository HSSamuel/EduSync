...
function buildStudentSelectFields(role) {
  ...
  const parentField =
    role === "Admin" ? "s.parent_id" : "NULL::int AS parent_id";
    ...
    return sendSuccess(res, {
      ...
      meta: {
        pagination: {
          ...
          limit: parsedLimit,
          totalPages:
            totalItems === 0 ? 0 : Math.ceil(totalItems / parsedLimit),
        },
      },
    });
  } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (parentQuery.rows.length === 0) {
        return sendError(res, {
          status: 404,
          message: "Parent account not found in your school.",
        });
      }
      ...
      if (parentQuery.rows[0].role !== "Parent") {
        return sendError(res, {
          status: 400,
          message: "The selected user is not a parent account.",
        });
      }
