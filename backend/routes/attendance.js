...
router.post("/", authorize, async (req, res) => {
  try {
    ...
    attendanceList.forEach((record) => {
      ...
      queryParams.push(record.student_id, date, record.status);
    });

    const insertQuery = `INSERT INTO attendance (student_id, date, status) VALUES ${values.join(", ")}`;
    await pool.query(insertQuery, queryParams);

    // --- FIX: BULK FETCH PARENT EMAILS ---
    const absentStudentIds = attendanceList
      .filter((record) => record.status === "Absent")
      .map((record) => record.student_id);

    if (absentStudentIds.length > 0) {
      // Fetch all required parents in ONE single query using ANY()
      ...
      const emailPromises = parentQuery.rows.map((parent) => {
        const emailHTML = `
